...
NoAudioCodecSimplexWithSD::NoAudioCodecSimplexWithSD(int input_sample_rate, int output_sample_rate,
                                                     gpio_num_t spk_bclk, gpio_num_t spk_ws,
                                                     gpio_num_t spk_dout, gpio_num_t spk_sd_pin,
    ...
    : NoAudioCodecSimplex(input_sample_rate, output_sample_rate, spk_bclk, spk_ws, spk_dout,
      ...
      spk_sd_pin_(spk_sd_pin) {
    if (spk_sd_pin_ != GPIO_NUM_NC) {
        gpio_config_t io_conf = {};
        io_conf.pin_bit_mask = (1ULL << spk_sd_pin_);
        io_conf.mode = GPIO_MODE_OUTPUT;
        io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
        io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
        io_conf.intr_type = GPIO_INTR_DISABLE;
        gpio_config(&io_conf);
        gpio_set_level(spk_sd_pin_, 0);  // Mute by default (SD LOW)
    }
}

void NoAudioCodecSimplexWithSD::EnableOutput(bool enable) {
    if (spk_sd_pin_ != GPIO_NUM_NC) {
        gpio_set_level(spk_sd_pin_, enable ? 1 : 0);  // HIGH = unmute, LOW = mute
    }
